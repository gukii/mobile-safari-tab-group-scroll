...
function getBaselineKey(storagePrefix) {
...
function getExpectedSafariViewportHeight(options) {
...
function isKeyboardLikelyOpen() {
    ...
    const hasTextInputFocused = activeTag === "INPUT" ||
        activeTag === "TEXTAREA" ||
        document.activeElement?.getAttribute("contenteditable") === "true";
    return hasTextInputFocused && lostHeight > 120;
}
function readStoredBaseline(key) {
    try {
...
function writeStoredBaseline(key, height) {
    try {
...
function emptyState(isSafari = false) {
    return {
...
function measure(options) {
    ...
    const plausibleDeltas = [baselineDelta, layoutViewportDelta, expectedViewportDelta].filter((delta) => delta >= options.tabGroupMinPx && delta <= options.tabGroupMaxPx);
...
export function useMobileSafariTabGroupScroll({ enabled = true, applyScrollCorrection = true, logDetection = true, offsetCssVariable = defaultOffsetCssVariable, scrollCssVariable = defaultScrollCssVariable, detectedClassName = defaultDetectedClassName, correctionDelaysMs = defaultCorrectionDelaysMs, ...measureOptions } = {}) {
    ...
    useEffect(() => {
        ...
        const scrollToOffset = (scrollPx) => {
            if (!applyScrollCorrection || scrollPx <= 0 || isKeyboardLikelyOpen())
                return;
        ...
        const update = () => {
            ...
            frame = window.requestAnimationFrame(() => {
                ...
                if (next.isLikelyTabGroupChromeVisible && logDetection && !loggedDetectionRef.current) {
                    ...
                    console.info(`Detected mobile Safari tab-group viewport offset: ${next.offsetPx}px`, {
...
//# sourceMappingURL=useMobileSafariTabGroupScroll.js.map